    ...
    <>
        <div className={` flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists?  " flex-row-reverse": "flex-row"} `}>
            ...
            <div className=' max-w-lg lg:max-w-none w-full md:text-center'>
                ...
                <h1 className=' text-4xl sm:text-3xl font-bold text-gradient mb-3'>{heading}</h1>
            ...
            <div className=' flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
                <img src={img} alt="imges" className={` w-auto object-fill transitions-theme ${ifExists?  "h-60 lg:h-56 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12 ": "h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40 rotate-[19deg] hover:rotate-12 "}`} />
            </div>
        </div>
    </>
  )
